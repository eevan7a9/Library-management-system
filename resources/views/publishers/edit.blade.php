...
@section('content')
<div class="container">
   <form action="{{ url('publishers/'.$publishers->id) }}" method="POST">
    @method("put")
    @csrf
        ...
        <div class="col-md-8">
            <div class="card text-left">
                <div class="card-body custom-bg-color-light">
                    <h4 class="card-title custom-font-cursive">{{ __("EDIT PUBLISHER") }}</h4>
                    <hr>
                    <div class="form-group">
                        <label for="publisher" class="font-weight-bold">Name</label>
                        <input type="text" name="name" id="publisher" class="form-control" value="{{ $publishers->name }}" aria-describedby="NameId">
